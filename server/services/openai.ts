...
/* ======== NEW: helper utilities for image parts ======== */
 ...
 *  - server-relative paths like /uploads/..., /depthmaps/..., /edgemaps/..., /gradientmaps/...
 * into an OpenAI "image_url" content part.
 * If server-relative and readable on disk, we inline as data: to avoid fetch issues.
 */
async function toImagePartFromUrl(u?: string | null) {
  ...
  // server-relative → try to read from disk and inline as data:
  if (url.startsWith("/")) {
    // map to server paths
    let abs: string | null = null;
    if (url.startsWith("/uploads/")) {
      abs = path.join(process.cwd(), "server", "uploads", url.replace(/^\/uploads\//, ""));
    } else if (url.startsWith("/depthmaps/")) {
      abs = path.join(process.cwd(), "server", "depthmaps", url.replace(/^\/depthmaps\//, ""));
    } else if (url.startsWith("/edgemaps/")) {
      abs = path.join(process.cwd(), "server", "edgemaps", url.replace(/^\/edgemaps\//, ""));
    } else if (url.startsWith("/gradientmaps/")) {
      abs = path.join(process.cwd(), "server", "gradientmaps", url.replace(/^\/gradientmaps\//, ""));
    }
    ...
    // If not found/readable, fall through and send as-is (the model may still be able to fetch)
    return { type: "image_url", image_url: { url } } as const;
  }

  // any other string (relative un-rooted) — send as-is
  return { type: "image_url", image_url: { url } } as const;
}
/* ======== END helpers ======== */
